...
`pd-ocr-ops` is a library (not a daemon) that runs in-process inside each pd-* app's
...
See `pd_ocr_ops/schemas/emit.py::PUBLIC_MODELS` for the registration surface.
